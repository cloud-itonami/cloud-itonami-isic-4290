(ns civilworks.store-contract-test
...
(deftest read-parity
  (doseq [[label s] (backends)]
    (testing label
      (is (= "Kotobuki Elementary School -- Running Track & Drainage Grading" (:name (store/site s "site-1"))))
      (is (= "JPN" (:jurisdiction (store/site s "site-1"))))
      (is (true? (:site-verified? (store/site s "site-1"))))
      (is (true? (:utility-survey-completed? (store/site s "site-1"))))
      (is (false? (:safety-concern-unresolved? (store/site s "site-1"))))
      (is (nil? (:excavation-depth-m (store/site s "site-1"))))
      (is (false? (:site-verified? (store/site s "site-3"))) "site-3 seeded not-yet-verified")
      (is (false? (:utility-survey-completed? (store/site s "site-4"))) "site-4 seeded with an incomplete utility survey")
      (is (= 3.5 (:excavation-depth-m (store/site s "site-5"))) "site-5 seeded above JPN's excavation-shoring trigger")
      (is (false? (:shoring-installed? (store/site s "site-5"))) "site-5 seeded with no shoring installed")
    ...
    (testing label
      (testing "log-site-record commits a patch and appends a site-record-log entry"
        (store/commit-record! s {:op :log-site-record :path ["site-1"]
                                 :value {:id "site-1" :utility-strike-detected? true}})
        (is (true? (:utility-strike-detected? (store/site s "site-1"))))
        (is (= "Kotobuki Elementary School -- Running Track & Drainage Grading" (:name (store/site s "site-1"))) "unrelated field preserved")
      ...
      (testing "order-supplies drafts a record and advances the sequence"
        (store/commit-record! s {:op :order-supplies :path ["site-1"]
                                 :value {:site-id "site-1" :items ["drainage-pipe-100mm"] :cost-usd 500}})
      ...
      (testing "sequences are jurisdiction-scoped, not global"
        (store/commit-record! s {:op :log-site-record :path ["site-7"]
                                 :value {:id "site-7" :utility-strike-detected? true}})
...
(deftest datomic-empty-store-is-usable
  (let [s (store/datomic-store)]
    ...
    (store/with-sites s {"x" {:id "x" :name "n" :jurisdiction "JPN"
                              ...
                              :safety-concern-unresolved? false :excavation-depth-m nil
                              :shoring-installed? nil :status :intake}})
