(ns civilworks.registry-test
  (:require [clojure.test :refer [deftest is]]
            [civilworks.registry :as r]))
...
(def sample-site
  {:id "site-1" :name "Kotobuki Elementary School -- Running Track & Drainage Grading" :jurisdiction "JPN"})

(deftest safety-concern-notice-cites-legal-basis-inline
  (let [doc (r/render-safety-concern-notice sample-site "JPN-SCF-000000" "掘削中に地中埋設ケーブルらしき物を確認")]
    (is (re-find #"JPN-SCF-000000" doc))
    (is (re-find #"労働安全衛生規則" doc))
    (is (re-find #"laws\.e-gov\.go\.jp" doc))
    (is (re-find #"埋設ケーブル" doc))
