(ns civilworks.governor-contract-test
  "The governor contract as executable tests -- the other-civil-
  engineering-coordination analog of `installation.governor-contract-
  test`/`demolition.governor-contract-test`. The single invariant under
  test:

    Civil Works Advisor never schedules a construction operation, files
    a safety-concern flag or a supply order the Civil Works Governor
    would reject; `:flag-safety-concern` NEVER auto-commits at any phase;
    `:log-site-record` (no direct capital/safety risk), `:schedule-
    construction-operation` (governor-clean, high-confidence --
    deliberately LOWER stakes than the sibling demolition/road-rail
    schedule ops, see `civilworks.governor` ns docstring) and `:order-
    supplies` (below the cost threshold) MAY auto-commit when clean; and
...
(deftest clean-log-site-record-auto-commits
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-site-record :subject "site-1" :patch {:id "site-1" :utility-strike-detected? true}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (true? (:utility-strike-detected? (store/site db "site-1"))) "SSoT actually updated")
...
(deftest clean-schedule-construction-operation-auto-commits
  (testing "site-1 is fully clean (verified, utility-surveyed, no excavation, high confidence) -- AUTO-COMMITS at phase 3, UNLIKE the sibling demolition/road-rail schedule ops"
    (let [[db actor] (fresh)
          res (exec-op actor "t2" {:op :schedule-construction-operation :subject "site-1" :project-type :outdoor-sports-facility :window {}} operator)]
      (is (= :commit (get-in res [:state :disposition])))
...
(deftest fabricated-jurisdiction-is-held
  (testing "site-2 (ATL, no spec-basis in civilworks.facts) -> HOLD, never reaches a human"
    (let [[db actor] (fresh)
          res (exec-op actor "t3" {:op :schedule-construction-operation :subject "site-2" :project-type :industrial-plant :window {}} operator)]
...
(deftest not-independently-verified-site-is-held
  (testing "site-3 has site-verified? false -> HARD hold, never reaches a human"
    (let [[db actor] (fresh)
          res (exec-op actor "t4" {:op :schedule-construction-operation :subject "site-3" :project-type :pipeline :window {}} operator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:site-not-verified} (-> (store/ledger db) first :basis)))
      (is (empty? (store/schedule-proposal-history db))))))

(deftest utility-survey-incomplete-is-held
  (testing "site-4 has utility-survey-completed? false -> HARD hold"
    (let [[db actor] (fresh)
          res (exec-op actor "t5" {:op :schedule-construction-operation :subject "site-4" :project-type :power-line :window {}} operator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:utility-survey-incomplete} (-> (store/ledger db) first :basis))))))

(deftest excavation-shoring-noncompliant-is-held
  (testing "site-5's excavation-depth-m (3.5) meets/exceeds JPN's 2m trigger with no shoring installed -> HARD hold, independent of proposal confidence"
    (let [[db actor] (fresh)
          res (exec-op actor "t6" {:op :schedule-construction-operation :subject "site-5" :project-type :industrial-plant :window {}} operator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:excavation-shoring-noncompliant} (-> (store/ledger db) first :basis))))))
...
(deftest unresolved-safety-concern-is-held
  (testing "site-6 has safety-concern-unresolved? true on file -> HARD hold, never reaches a human"
    (let [[db actor] (fresh)
          res (exec-op actor "t7" {:op :schedule-construction-operation :subject "site-6" :project-type :pipeline :window {}} operator)]
      (is (= :hold (get-in res [:state :disposition])))
...
(deftest qualitative-jurisdiction-never-fabricates-a-numeric-hold-and-still-auto-commits-when-clean
  (testing "site-8 (DEU/EU, qualitative) -- excavation-shoring-noncompliant? never fires there; clean + high confidence -> AUTO-COMMITS"
    (let [[db actor] (fresh)
          res (exec-op actor "t8" {:op :schedule-construction-operation :subject "site-8" :project-type :pipeline :window {}} operator)]
      (is (= :commit (get-in res [:state :disposition])))
      (is (= "DEU-SCH-000000" (get (first (store/schedule-proposal-history db)) "record_id"))))))

(deftest usa-cross-jurisdiction-happy-path-at-trigger-but-compliant-auto-commits
  (testing "site-7 (USA, quantitative 1.5m/5ft trigger), depth=1.5m but shoring-installed? true -- compliant, clean, high confidence -> AUTO-COMMITS"
    (let [[db actor] (fresh)
          res (exec-op actor "t9" {:op :schedule-construction-operation :subject "site-7" :project-type :power-line :window {}} operator)]
      (is (= :commit (get-in res [:state :disposition])))
  ...
  (testing "an uncovered-jurisdiction low-confidence branch already HARD-holds (see fabricated-jurisdiction-is-held); this test proves the SOFT confidence-floor path independently via governor/check directly, since the mock advisor's own clean branch is always high-confidence"
    (let [db (store/seed-db)
          ...
          proposal {:summary "s" :rationale "r" :cites ["x"] :effect :propose
                    :value {:site-id "site-1" :spec-basis "https://example.com"} :stake :schedule-construction-operation :confidence 0.4}
  ...
  (testing "site-1 is fully clean -- :flag-safety-concern STILL always interrupts, unconditionally"
    (let [[db actor] (fresh)
          r1 (exec-op actor "t10" {:op :flag-safety-concern :subject "site-1"
                                   ...
                                   :concern-description "possible buried cable observed near drainage cut"} operator)]
...
(deftest flag-safety-concern-triggers-notification-only-after-approval
  (let [[_db actor] (fresh)
        r1 (exec-op actor "t11" {:op :flag-safety-concern :subject "site-1"
                                 :concern-type :utility-strike :concern-description "suspected unmarked gas conduit"} operator)]
...
(deftest order-supplies-below-threshold-auto-commits
  (let [[db actor] (fresh)
        res (exec-op actor "t12" {:op :order-supplies :subject "site-1"
                                  :items ["drainage-pipe-100mm"] :cost-usd 800 :vendor "Local Civil Supply Co."} operator)]
    (is (= :commit (get-in res [:state :disposition])))
...
(deftest order-supplies-above-threshold-escalates
  (let [[db actor] (fresh)
        r1 (exec-op actor "t13" {:op :order-supplies :subject "site-1"
                                 :items ["trench-shoring-box"] :cost-usd 9000 :vendor "Heavy Civil Rentals"} operator)]
...
(deftest forbidden-action-class-markers-are-permanent-hard-violations
  (doseq [marker [:heavy-equipment-control? :direct-actuation? :finalizes-structural-completion-sign-off?]]
    (testing marker
      (let [db (store/seed-db)
            request {:op :schedule-construction-operation :subject "site-1"}
            proposal {:summary "s" :rationale "r" :cites ["x"] :effect :propose
...
(deftest every-decision-leaves-one-ledger-fact
  (testing "write-only-through-ledger: N operations -> N ledger facts"
    (let [[db actor] (fresh)]
      (exec-op actor "a" {:op :log-site-record :subject "site-1" :patch {:id "site-1" :utility-strike-detected? true}} operator)
      (exec-op actor "b" {:op :schedule-construction-operation :subject "site-2" :project-type :industrial-plant :window {}} operator)
...
(deftest approver-rejection-is-held-not-committed
  (let [[db actor] (fresh)
        r1 (exec-op actor "t15" {:op :flag-safety-concern :subject "site-1"
                                 :concern-type :utility-strike :concern-description "test"} operator)]
    (is (= :interrupted (:status r1)))
...
(deftest schedule-construction-operation-is-auto-eligible-at-phase-3-unlike-siblings
  (testing "structural invariant proving the deliberate difference from demolition.phase/roadrail.phase"
    (is (= :commit (:disposition (phase/gate 3 {:op :schedule-construction-operation} :commit)))
        ":schedule-construction-operation MAY auto-commit at phase 3 when the governor is clean -- see civilworks.governor/civilworks.phase ns docstrings")))
