(ns civilworks.store
  "SSoT for the other-civil-engineering-project-coordination actor, behind
  ...
  (test/civilworks/store_contract_test.clj), which is the whole point:
  the actor, the Civil Works Governor and the audit ledger never know
  ...
  trenching/foundation/installation phases, may accumulate multiple
  safety-concern flags, may place multiple supply orders) BECAUSE this
  actor never actually dispatches heavy equipment/crew, authorizes
  equipment use, or signs off on structural completion -- it only ever
  proposes, logs and schedules (`:effect :propose` unconditionally, see
  `civilworks.governor` ns docstring). The site's own `:site-verified?`
  / `:utility-survey-completed?` / `:safety-concern-unresolved?` /
  `:excavation-depth-m` / `:shoring-installed?` ground-truth fields are
  what the Civil Works Governor independently re-checks before a
  ...
  which construction-operation schedule was proposed, which safety
  ...
  immutable log."
  (:require [civilworks.registry :as registry]
...
(defprotocol Store
  ...
  (all-sites [s])
  (ledger [s])
  (site-record-log-history [s] "the append-only site-record-log history (civilworks.registry drafts)")
...
(defn demo-data
  ...
  outdoor-sports-facility drainage/grading -- no excavation-depth concern
  involved), the uncovered-jurisdiction / not-verified / utility-survey-
  incomplete / excavation-shoring-noncompliant / unresolved-safety-
  concern failure modes (site-2..site-6), a cross-jurisdiction (USA)
  happy path where the site is AT the numeric trigger depth but
  compliant because shoring is already installed (site-7), and the
  honestly-qualitative (DEU/EU) jurisdiction that never fabricates a
  numeric trigger (site-8) -- so the actor + tests run offline."
  []
  {:sites
   {"site-1" {:id "site-1" :name "Kotobuki Elementary School -- Running Track & Drainage Grading"
              :jurisdiction "JPN" :project-type :outdoor-sports-facility
              :site-verified? true :utility-survey-completed? true
              :utility-strike-detected? false :safety-concern-unresolved? false
              :excavation-depth-m nil :shoring-installed? nil
    ...
    "site-2" {:id "site-2" :name "Atlantis Processing Complex -- Industrial Plant Piping"
              :jurisdiction "ATL" :project-type :industrial-plant
              :site-verified? true :utility-survey-completed? true
              :utility-strike-detected? false :safety-concern-unresolved? false
              :excavation-depth-m 2.0 :shoring-installed? true
              :safety-contacts []
              :status :intake}
    "site-3" {:id "site-3" :name "鈴木パイプライン -- 天然ガス導管敷設工事"
              :jurisdiction "JPN" :project-type :pipeline
              :site-verified? false :utility-survey-completed? false
              :utility-strike-detected? false :safety-concern-unresolved? false
              :excavation-depth-m nil :shoring-installed? nil
              :safety-contacts [{:name "Sato (site supervisor)" :email "sato@example.com" :phone "+819000000003"}]
              :status :unverified}
    "site-4" {:id "site-4" :name "田中変電所 -- 送電線鉄塔基礎工事"
              :jurisdiction "JPN" :project-type :power-line
              :site-verified? true :utility-survey-completed? false
              :utility-strike-detected? nil :safety-concern-unresolved? false
              :excavation-depth-m 1.0 :shoring-installed? false
              :safety-contacts [{:name "Ito (site supervisor)" :email "ito@example.com" :phone "+819000000004"}]
              :status :survey-pending}
    "site-5" {:id "site-5" :name "ことぶき工業団地 -- プラント基礎掘削工事"
              :jurisdiction "JPN" :project-type :industrial-plant
              :site-verified? true :utility-survey-completed? true
              :utility-strike-detected? false :safety-concern-unresolved? false
              :excavation-depth-m 3.5 :shoring-installed? false
              :safety-contacts [{:name "Watanabe (site supervisor)" :email "watanabe@example.com" :phone "+819000000005"}]
              :status :shoring-pending}
    "site-6" {:id "site-6" :name "山田パイプライン -- 上水道導管敷設工事"
              :jurisdiction "JPN" :project-type :pipeline
              :site-verified? true :utility-survey-completed? true
              :utility-strike-detected? true :safety-concern-unresolved? true
              :excavation-depth-m nil :shoring-installed? nil
              ...
              :status :concern-open}
    "site-7" {:id "site-7" :name "Riverside Substation -- Transmission Tower Foundation"
              :jurisdiction "USA" :project-type :power-line
              :site-verified? true :utility-survey-completed? true
              :utility-strike-detected? false :safety-concern-unresolved? false
              :excavation-depth-m 1.5 :shoring-installed? true
              :safety-contacts [{:name "Casey (site supervisor)" :email "casey@example.com" :phone "+15550000002"}]
              :status :ready-to-schedule}
    "site-8" {:id "site-8" :name "Alt-Industriepark -- Rohrleitungsbau"
              :jurisdiction "DEU" :project-type :pipeline
              :site-verified? true :utility-survey-completed? true
              :utility-strike-detected? false :safety-concern-unresolved? false
              :excavation-depth-m 2.5 :shoring-installed? true
...
(def ^:private site-spec
  ...
  {:id                             {:attr :site/id}
   ...
   :project-type                   {:attr :site/project-type}
   :site-verified?                 {:attr :site/site-verified? :coerce boolean}
   :utility-survey-completed?      {:attr :site/utility-survey-completed? :coerce boolean}
   :utility-strike-detected?       {:attr :site/utility-strike-detected? :coerce boolean}
   :safety-concern-unresolved?     {:attr :site/safety-concern-unresolved? :coerce boolean}
   :excavation-depth-m             {:attr :site/excavation-depth-m}
   :shoring-installed?             {:attr :site/shoring-installed? :coerce boolean}
