(ns civilworks.operation
  "OperationActor -- one other-civil-engineering-project coordination
  operation = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (Civil Works Advisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Civil Works
  ...
  One graph run = one other-civil-engineering-project coordination
  operation (intake -> advise -> govern -> decide -> commit | hold |
  approval). No unbounded inner loop -- each operation is auditable and
  checkpointed.
  ...
  `:flag-safety-concern` ALWAYS reaches this node -- see `civilworks.
  phase`/`civilworks.governor` ns docstrings. `:log-site-record`,
  `:schedule-construction-operation` and `:order-supplies` (below the
  cost threshold) MAY auto-commit at phase 3 when the governor is clean
  -- see `civilworks.phase` ns docstring.

  Every committed record carries `:effect :propose` (see `civilworks.
  ...
  `civilworks.notify`) to the site's `:safety-contacts` roster -- always
...
(defn build
  ...
  `civilworks.store/Store`).
  opts:
    :advisor      -- a `civilworks.advisor/Advisor` (default: mock-advisor)
    :notifier     -- a `civilworks.notify/Notifier` (default: mock-notifier)
  ...
  (-> (g/state-graph
      ...
      ;; Civil Works Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Civil Works Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
