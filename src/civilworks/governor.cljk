(ns civilworks.governor
  "Civil Works Governor -- the independent compliance layer named in
  `blueprint.edn` (`:itonami.blueprint/governor :civilworks-governor`)
  that earns the Civil Works Advisor the right to commit. The LLM has no
  notion of civil-engineering excavation/buried-utility safety law,
  whether a site's own recorded pre-work utility survey / excavation-
  shoring measures actually satisfy its jurisdiction's requirements, or
  when a proposal has quietly drifted outside this actor's charter into
  heavy-equipment control or structural-completion sign-off, so this
  MUST be a separate system able to *reject* a proposal and fall back to
  HOLD -- the other-civil-engineering analog of `cloud-itonami-isic-
  4311`'s Demolition Governor / `cloud-itonami-isic-4329`'s Installation
  Governor.

  ## Scope -- this is a COORDINATION-ONLY actor

  This actor NEVER controls heavy equipment and NEVER finalizes a
  structural-completion sign-off -- that authority is the site
  supervisor / building official's EXCLUSIVELY. Every proposal this
  actor's Civil Works Advisor can produce carries `:effect :propose` and
  NOTHING else -- committing a proposal here means 'this coordination
  artifact is now logged/scheduled/flagged/ordered', never 'a crew or
  piece of equipment was dispatched' or 'a structural-completion sign-off
  is authorized'. Checks 1-4 below encode this scope as STRUCTURAL,
  permanent HARD holds -- not policy that could be relaxed by a future
  phase.
  ...
  CANNOT override them.
    ...
    5. Legal-basis missing       -- for `:schedule-construction-
                                     ...
                                     OFFICIAL source (`civilworks.
                                     facts`), or invent one for an
                                     uncovered jurisdiction?
    6. Utility survey incomplete -- for `:schedule-construction-
                                     operation`, has the site's own
                                     recorded `:utility-survey-completed?`
                                     ground-truth field (the pre-work
                                     buried-utility/ground-condition
                                     survey) actually been set true?
    7. Excavation-shoring
       noncompliant               -- for `:schedule-construction-
                                     operation` in a `:quantitative`-
                                     threshold jurisdiction (JPN/USA),
                                     INDEPENDENTLY recompute whether the
                                     site's own recorded `:excavation-
                                     depth-m` / `:shoring-installed?`
                                     fields meet its jurisdiction's
                                     regulatory excavation-shoring
                                     trigger (`civilworks.facts/
                                     excavation-shoring-noncompliant?`)
                                     -- needs no proposal inspection at
                                     all. DELIBERATELY does not fire for
  ...
  the human may approve.
    ...
    - `:flag-safety-concern` is UNCONDITIONALLY a member of `high-stakes`
      ...
      confidence or governor cleanliness. Surfacing a buried-utility-
      strike, excavation-collapse or structural concern is exactly the
    ...
    - `:schedule-construction-operation` is DELIBERATELY **NOT** a
      permanent member of `high-stakes` -- the same structural choice
      `installation.governor`/`finishing.governor` make for their own
      schedule ops (as opposed to `demolition.governor`/`roadrail.
      governor`'s schedule ops, which ARE unconditional `high-stakes`
      members). Once the eight HARD checks above are clean (site
      verified, utility survey complete, no excavation-shoring
      violation, no unresolved concern, legal basis cited) AND
      confidence clears `confidence-floor`, it is treated the SAME as
      `:order-supplies`: it MAY auto-commit at phase 3 (see `civilworks.
      phase` ns docstring). It still escalates like any other write
      whenever confidence is low or the rollout phase does not yet allow
      auto-commit. The HARD checks (1-8) always apply regardless of
      phase or confidence -- only the routing to a human vs. auto-commit
      changes."
  (:require [civilworks.facts :as facts]
...
(def high-stakes
  ...
  `:schedule-construction-operation` is ALSO deliberately NOT a member --
...
(defn- forbidden-action-class-violations
  "A proposal's `:value` must never carry a heavy-equipment-control /
...
(defn- legal-basis-missing-violations
  ...
  OFFICIAL source -- never invent a jurisdiction's utility-survey/
  excavation-shoring requirements. See ns docstring check 5."
  [{:keys [op]} proposal]
  (when (= op :schedule-construction-operation)
    (let [value (:value proposal)]
      (when (or (empty? (:cites proposal))
        ...
        [{:rule :no-legal-basis
          :detail "公式legal-basisの引用が無い提案は土木工事スケジュール提案として扱えない"}]))))

(defn- utility-survey-incomplete-violations
  "For `:schedule-construction-operation`, the site's own recorded
  `:utility-survey-completed?` ground-truth field must be true. See ns
  docstring check 6."
  [{:keys [op subject]} st]
  (when (= op :schedule-construction-operation)
    (let [a (store/site st subject)]
      (when-not (true? (:utility-survey-completed? a))
        [{:rule :utility-survey-incomplete
          :detail (str subject " は事前調査（埋設物等のutility survey）が完了記録されていない状態での土木工事スケジュール提案")}]))))

(defn- excavation-shoring-noncompliant-violations
  "For `:schedule-construction-operation`, INDEPENDENTLY recompute
  whether the site's own recorded excavation-depth/shoring fields meet
  its jurisdiction's regulatory excavation-shoring trigger via
  `civilworks.facts/excavation-shoring-noncompliant?`. Fires ONLY when
  that returns `true` (a :quantitative jurisdiction confirmed
  noncompliant) -- `:qualitative`/`nil` never trip this HARD check. See
  ns docstring check 7."
  [{:keys [op subject]} st]
  (when (= op :schedule-construction-operation)
    (let [a (store/site st subject)]
      (when (true? (facts/excavation-shoring-noncompliant? (:jurisdiction a) a))
        [{:rule :excavation-shoring-noncompliant
          :detail (str subject " の掘削深さ実測値（" (:excavation-depth-m a)
                      "m）が法定土止め支保工トリガー深さ以上で、土止め支保工等の防護措置が未設置")}]))))
    ...
    (let [a (store/site st subject)]
      (when (true? (:safety-concern-unresolved? a))
        [{:rule :unresolved-safety-concern
          :detail (str subject " は未解決の安全性懸念（埋設物損傷/掘削崩壊/構造）がある状態での土木工事スケジュール提案")}]))))

(defn check
  "Censors a Civil Works Advisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (unknown-op-violations request)
                           ...
                           (utility-survey-incomplete-violations request st)
                           (excavation-shoring-noncompliant-violations request st)
