(ns civilworks.notify
  ...
  other-civil-engineering-domain analog of `installation.notify`/
  `demolition.notify`.
  ...
  safety officer, NOT the crew directly -- this actor coordinates, it
  does not dispatch a crew) over BOTH channels, isolating one contact's
  send failure from every other contact's.
  ...
  human first (see `civilworks.phase`/`civilworks.governor` ns
  docstrings), so by the time `civilworks.operation`'s `:commit` node
  ...
  `civilworks.notify` itself runnable unmodified on JVM Clojure,
